...
export const beautifulSuggestions: Record<ProjectMode, string[]> = {
  photo: [
    "верх белый матовый, низ дуб натуральный, столешница светлый камень — только перекрасить, ничего не добавлять",
    "фасады графит матовый, столешница белый мрамор — менять только материалы существующей мебели",
    "верх белый глянец, низ антрацит, столешница бетон — только поверхности, без новых предметов",
    "фасады бежевые матовые, столешница светлый камень — перекрасить аккуратно и дорого",
  ],
  sketch: [
    "реалистичная визуализация строго по эскизу: белые матовые фасады, дуб, камень — без новых предметов",
    "современная кухня по эскизу: графит низ, белый верх, мрамор — точно по конфигурации эскиза",
  ],
};
